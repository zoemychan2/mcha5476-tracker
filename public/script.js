// Foundation format of this Javascript was based on DECO2017 JS Objects Input and event handling Scrimba https://scrimba.com/scrim/coa064ba08c96c1639bc9531b
...
function displayTrackedbobaorders() {
    ...
    if (localTracked !== null){  // If localTracked is not null
        ...
        localTracked.forEach((bobatrackform)=>{ // running through bobatrackform, for each element in the localTacked array
            ...
            // Changing the format of date, inspired by mdn (2023) https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Intl/DateTimeFormat
            ...
            delButton.addEventListener('click', function (event) {
                listContainer.remove(); // When delete button is pressed, item element is removed 
                dateDeletecontainer.remove(); // The date container is removed as well
                
                localTracked = localTracked.filter((listContainer)=> listContainer.id !==bobatrackform.id); // when pressing the delete button, localTracked checks if item.id equals to bobatrackform.id and removes it 
            ...
            generatedImage.src = bobatrackform.image; // Random autogenerated image picked from autoImages array
...
function addBobaorder(bobaorder, bobashop, bobasize, bobaprice, bobarating, bobacomments, date) {
    ...
    let bobatrackform = {
       ...
       // Properties of bobatrackform
        ...
        image: getRandomImage(autoImages), 
    }
